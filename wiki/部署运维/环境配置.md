...
- [app.js](file://miniprogram/app.js)（`globalData.env`）
- [safeLog.js](file://miniprogram/utils/safeLog.js)
...
## 简介
本指南面向“萌芽季”小程序的开发者与运维人员，系统化说明开发、测试、生产三类环境的配置方法与差异；详解项目配置文件 project.config.json 的关键参数；说明 **云环境 ID 在 `app.js` 的 `globalData.env` 中配置**（历史文件 `miniprogram/envList.js` 已移除）；说明云开发环境变量与云函数部署流程；并提供 project.private.config.json 的私有配置管理策略与环境隔离最佳实践。
...
- 云环境 ID 由 `app.js` 中 `wx.cloud.init({ env: globalData.env })` 指定，与云函数 `wx-server-sdk` 动态环境一致。

```mermaid
graph TB
subgraph "小程序前端"
APP["app.js<br/>globalData.env + 云初始化"]
...
CF_EMAIL["cloudfunctions/sendFeedbackEmail/index.js<br/>反馈邮件 SMTP"]
DB["云数据库/存储"]
end
APP --> API
API --> CF_LOGIN
CF_LOGIN --> DB
CF_LOGIN -. submitFeedback 内调用 .-> CF_EMAIL
CFG -.-> APP
PRIVCFG -.-> CFG
```
...
- 环境与云初始化
  - app.js：小程序启动时 `wx.cloud.init`，云环境 ID 使用 `globalData.env`（勿再依赖已删除的 `envList.js`）。
- 云函数
...
### 云环境 ID（app.js）
- 当前做法
  - 在 `miniprogram/app.js` 的 `globalData.env` 中填写云开发环境 ID，`wx.cloud.init({ env: ... })` 与之保持一致。
  - 历史文件 `miniprogram/envList.js` 已从仓库移除；文档或旧图若仍引用该路径，应改为 `app.js`。
- 多环境建议
  - 可按构建脚本或 Git 分支切换不同 `globalData.env` 值；云函数继续使用 `cloud.DYNAMIC_CURRENT_ENV` 与小程序当前环境对齐。
...
- 业务职责
  ...
  - sendFeedbackEmail/index.js：SMTP 发送反馈邮件；由 `login` 的 `submitFeedback` 在服务端调用。
- 数据访问模式
...
### 云函数部署
- 推荐：在微信开发者工具中右键云函数目录「上传并部署：云端安装依赖」；或在 CI/CD 中使用 [云开发 CLI](https://docs.cloudbase.net/cli-v1/intro) 上传。
- 仓库内 **不再包含** `uploadCloudFunction.sh`；若文档仍提及，请以工具/CLI 为准。
...
- 环境切换问题
  - 修改 `app.js` 中 `globalData.env` 后重新编译；确认云函数已部署到同一云环境。
...
## 结论
通过 project.config.json 与 project.private.config.json 的合理配置、在 app.js 中明确云环境 ID、以及云函数使用 DYNAMIC_CURRENT_ENV，可以实现开发、测试、生产三环境的清晰隔离与高效协作。建议在团队内固化环境变量管理流程与部署方式，持续完善权限规则与日志监控。
...
- 开发环境
  ...
  - app.js：将 `globalData.env` 设为开发用云环境 ID。
  - 云函数：部署到该环境，开启详细日志。
- 测试环境
  - project.config.json：启用压缩与站点地图校验。
  - app.js：`globalData.env` 切换为测试环境 ID（或通过构建注入）。
  - 云函数：部署到测试环境，开启必要的安全规则。
- 生产环境
  - project.config.json：严格开启 urlCheck、minified、minifyWXSS/WXML。
  - app.js：`globalData.env` 为生产环境 ID。
  - 云函数：部署到生产环境，严格限制权限与超时。

### 环境切换最佳实践
- 统一管理
  - 以 `app.js` 的 `globalData.env`（及发布流水线注入）为小程序环境真源；云函数依赖 `DYNAMIC_CURRENT_ENV` 自动对齐。
- 自动化
