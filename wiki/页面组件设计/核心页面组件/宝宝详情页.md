...
宝宝详情页是微信小程序"萌芽季"项目中的核心功能模块，专门用于展示和分析宝宝的成长数据。该页面集成了ECharts图表组件，实现了身高体重曲线的可视化展示，包括标准曲线对比分析、数据点标注、交互式缩放等功能。页面采用响应式设计，支持多种设备尺寸，并提供了完整的权限控制机制。
...
1. **月龄**：由出生日与记录日计算整月月龄，用于在图表上与标准曲线、实测数据对齐；展示范围上界与 **7 岁以下**定义一致（见 `GROWTH_CHART_X_MAX_MONTH`）。
2. **标准曲线**：采用 **WS/T 423—2022** 附录表 **A.3 / A.4**（身高）与 **A.1 / A.2**（体重）中的 **P3、P50、P97**，仅使用表中给出的年龄刻度（44 点），**不使用** WHO 或其它来源。
...
1. **标准曲线对比**：基于 **WS/T 423—2022** 附录百分位（P3/P50/P97）的对比分析
