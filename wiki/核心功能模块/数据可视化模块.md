...
- 数据准备流程
  ...
  - 标准曲线仅含 **附录表年龄刻度** 对应的点（`WS423_STANDARD_MONTHS`，共 44 个月龄值，末项 81）；宝宝实测点仍按记录日计算整月月龄
- 图表配置
...
### 标准曲线计算逻辑
- 数据来源：**国家卫健委 WS/T 423—2022**《7 岁以下儿童生长标准》附录 **表 A.1（男体重）、A.2（女体重）、A.3（男身高）、A.4（女身高）**，列 **P3、P50、P97**
- 年龄刻度：与 PDF 附录一致，共 **44** 行（0 月～11 月、1 岁～1 岁 11 月、2 岁及 2 岁 3/6/9 月…直至 **6 岁 9 月 = 81 月**）；**不为**表中未列出的整月编造数值
- 实现要点（`baby-detail.js`）
  - 常量 `WS423_STANDARD_MONTHS` 与各性别 `WS423_*_HEIGHT/WEIGHT_P{3,50,97}` 等长
  - `getChartOptions` 将标准曲线生成为 `[[月龄, y], ...]`，**仅**上述 44 个点；`smooth: false`（分段折线，不引入插值点）
  - 横轴上限 `GROWTH_CHART_X_MAX_MONTH = 83`（未满 84 月龄）；Tooltip 中 Y 值保留一位小数
- 数据格式
  - 标准曲线：`[[月龄, 百分位数值], ...]`
  - 实测曲线：`[[round(月龄), 测量值], ...]`
...
- 坐标轴
  ...
  - Y 轴：数值型，按区间对齐最小/最大值
- 系列
  - 标准曲线（P3/P50/P97）：**折线**（`smooth: false`），虚线/实线区分
  - 实际数据：平滑线，圆点标记，宽度适中
