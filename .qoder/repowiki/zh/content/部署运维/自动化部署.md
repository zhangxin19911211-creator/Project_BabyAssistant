...
## 简介
本指南面向“宝宝助手”小程序的云函数自动化部署场景，聚焦于 uploadCloudFunction.sh 脚本的使用与参数说明、云函数打包与上传流程（以 login 与 sendFeedbackEmail 为例）、依赖管理策略、部署前检查清单以及常见失败原因与解决方案。文档同时给出增量更新与全量部署的差异说明，帮助团队在不同阶段选择合适的部署策略。
