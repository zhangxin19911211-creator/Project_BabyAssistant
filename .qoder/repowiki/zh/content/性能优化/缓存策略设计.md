...
- API 工具层：统一封装用户态、等待登录、数据库访问、云函数调用。
...
API->>LS : 读取用户标识(openid)
alt 需要登录
API->>CF : 调用云函数获取/刷新用户信息
CF-->>API : 返回用户信息
API->>LS : 写入/更新本地存储
end
API->>CF : 权限校验/复杂查询(绕过客户端权限)
CF->>DB : 读写数据(事务/聚合)
DB-->>CF : 返回结果
CF-->>API : 返回结果
API-->>Page : 渲染数据
...
- 本地缓存：利用本地存储承载用户态与轻量数据，降低重复请求成本。
...
## 结论
本项目通过“前端本地缓存 + 云函数统一权限与复杂查询 + 数据库存储”的组合，实现了清晰的多层级缓存与一致性保障。建议在此基础上进一步引入显式的接口缓存层（如 LRU 内存缓存）、标准化缓存键与过期策略、埋点统计缓存命中率与性能指标，并完善自动清理与预热机制，以持续提升用户体验与系统稳定性。

## 附录

### 多层级缓存架构设计要点
- 内存缓存：App 全局用户态、页面级轻量数据。
- 本地缓存：用户标识、最近一次查询结果片段。
