...
- 页面逻辑：首页与详情页通过封装好的 API 进行数据拉取与展示，部分场景存在串行多次请求的问题，适合引入优化策略。
...
### 数据获取与权限校验
- 统一入口：所有业务数据通过 api.js 的函数封装，集中处理登录等待、错误捕获与结果解析
- 权限校验：针对不同操作（如删除记录、更新头像）在前端与云函数两端进行权限判断
- 数据查询：支持按宝宝 ID 查询记录、按家庭 ID 查询记录等，云函数侧进行权限与可见性校验

```mermaid
flowchart TD
Start(["开始"]) --> GetUser["获取当前用户(openid)"]
GetUser --> NeedLogin{"是否已登录?"}
NeedLogin -- 否 --> WaitLogin["等待登录完成(最大等待5秒)"]
WaitLogin --> LoginOK{"登录成功?"}
LoginOK -- 否 --> Fail["返回空/抛错"]
LoginOK -- 是 --> CallAPI["调用云函数(action)"]
NeedLogin -- 是 --> CallAPI
CallAPI --> Parse["解析结果(success/error)"]
Parse --> Done(["结束"])
Fail --> Done
```
...
API-->>IDX : 返回宝宝列表
loop 对每个宝宝
IDX->>API : getLatestRecord(babyId)
API-->>IDX : 返回最新记录
end
IDX->>API : getFamilies()
API-->>IDX : 返回家庭列表
...
- [miniprogram/utils/api.js:288-297](file://miniprogram/utils/api.js#L288-L297)
...
### 请求合并与批量化
- 现状：首页逐条获取最新记录，存在多次往返
- 建议：
  - 在云函数中提供批量查询接口（如按宝宝 ID 列表一次性返回各宝宝最新记录），前端改为一次调用
  - 将“获取家庭列表”与“获取宝宝列表”合并为一次云函数调用，减少往返次数
  - 将“获取记录列表”与“获取标准曲线数据”在前端本地计算，避免重复网络请求

章节来源
- [miniprogram/pages/index/index.js:14-52](file://miniprogram/pages/index/index.js#L14-L52)
- [miniprogram/utils/api.js:242-286](file://miniprogram/utils/api.js#L242-L286)
- [cloudfunctions/login/index.js:50-92](file://cloudfunctions/login/index.js#L50-L92)

### 缓存策略
- 前端缓存：
  - 登录态缓存：在 app.js 中持久化 openid，避免重复登录
  - 结果缓存：对高频读取的数据（如家庭列表、宝宝列表）在页面级缓存，结合生命周期与刷新策略
- 云函数缓存：
  - 对静态数据（如标准曲线）可在云函数中缓存热点数据，降低重复计算与查询成本
  - 对频繁访问的查询结果设置短期缓存（如内存缓存），注意与数据库一致性

章节来源
- [miniprogram/app.js:39-43](file://miniprogram/app.js#L39-L43)
- [miniprogram/utils/api.js:44-111](file://miniprogram/utils/api.js#L44-L111)

### 连接复用与并发控制
- 连接复用：微信小程序云函数调用天然复用底层连接，无需额外配置
- 并发控制：
  - 页面加载阶段避免同时发起多个高代价请求，采用串行或分批策略
...
### 云函数调用优化
- 批量 API 调用：在云函数内部合并数据库查询，减少跨进程往返
...
## 结论
本项目已具备良好的云函数聚合与权限控制基础。建议围绕“请求合并、缓存、并发控制、监控与异常处理”四个方面持续优化，结合弱网与真实设备测试，逐步提升网络性能与稳定性，最终达到更优的用户体验与更低的运营成本。
