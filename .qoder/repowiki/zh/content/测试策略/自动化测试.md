...
## 项目结构
项目采用"小程序前端 + 云函数后端 + 手动测试"的三层架构，云函数位于 cloudfunctions 目录下，小程序源码位于 miniprogram 目录；根目录包含构建与上传脚本、测试配置等。
...
A --> D["uploadCloudFunction.sh 云函数上传脚本"]
A --> E["package.json 脚本与依赖"]
A --> F["project.config.json 项目配置"]
```
...
- **构建与忽略**：package.json 定义 npm 脚本占位。
...
Dev->>IDE : 打开项目并登录云开发
IDE->>Console : 打开调试控制台
Console->>Cloud : 调用云函数接口
Cloud->>DB : 执行数据库操作
DB-->>Cloud : 返回查询结果
Cloud-->>Console : 返回测试数据
Console-->>Dev : 显示测试报告
```

**图表来源**
- [cloudfunctions/login/index.js:73-200](file://cloudfunctions/login/index.js#L73-L200)
- [miniprogram/utils/api.js:101-131](file://miniprogram/utils/api.js#L101-L131)
...
- [miniprogram/utils/util.js:29-59](file://miniprogram/utils/util.js#L29-L59)
...
**章节来源**
- [miniprogram/utils/util.js:1-99](file://miniprogram/utils/util.js#L1-L99)
...
- **测试系统**：依赖 util.js 工具函数、api.js API 封装。
- **CI 依赖**：npm 脚本与构建产物。
...
**章节来源**
- [cloudfunctions/login/index.js:145-200](file://cloudfunctions/login/index.js#L145-L200)
