...
- **成长曲线分析**：通过图表直观展示宝宝的成长趋势；标准线依据 **WS/T 423—2022《7 岁以下儿童生长标准》** 附录表 **A.1～A.4** 的 **P3 / P50 / P97**（身高/体重、男/女），横轴月龄与表中年龄刻度一致（共 44 个年龄点，末行「6岁9月」= 81 月；未满 7 岁展示至 83 月）
...
### v3.1.0
- **生长标准曲线**：按 WS/T 423—2022 附录 **表 A.1～A.4** 仅使用表中 **P3/P50/P97** 与 **年龄刻度**（44 行），不再使用逐月内插或 85 点补全；标准折线为分段折线（非平滑插值）
- **反馈图片**：校验本地文件大小时使用 `wx.getFileSystemManager().getFileInfo`（替代已废弃的 `wx.getFileInfo`）
- **文档**：同步更新 `wiki/` 中数据可视化、宝宝详情等说明
